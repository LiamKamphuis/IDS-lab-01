...
There are 348 tourist attractions
...
NA stands for not available, meaning there is no mean or median for the n_2021 data set available to calculate. This is due to some of the data entries being labelled as NA
...
The input argument na.rm determine how "NA" data entries are handled in the calculation of the mean/median. If "TRUE", population statistics will be calculated with available entries not labelled "NA".
...
This suggests a large positive "skewness," meaning there is a larger differences in values above the mean than below, causing the data to appear skewed more to the right and pulling the mean higher. The median is less affected by extremely high values
...
visitors_with_nations %>%
  group_by(nation) %>%
  summarise(
    count_free = sum(admission == "Free", na.rm = TRUE),
    total = n(),
    proportion_free = count_free / total
  )
...
visitors_with_nations %>%
  mutate(
    pct_change = ((n_2022 - n_2021) / n_2021) * 100
  ) %>%
